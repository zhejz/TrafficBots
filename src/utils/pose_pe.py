...
from utils.pos_emb import PositionalEmbedding, PositionalEmbeddingRad
...
class PosePE(nn.Module):
    def __init__(self, mode: str, pe_dim: int = 256, theta_xy: float = 1e3, theta_cs: float = 1e1):
        ...
        if self.mode == "xy_dir":
            ...
            self.out_dim = 7
        elif self.mode == "pe_xy_unit_dir":
            self.out_dim = pe_dim + 2
            self.pe_xy = PositionalEmbedding(dim=pe_dim // 2, theta=theta_xy)
    ...
    def forward(self, xy: Tensor, dir: Tensor):
        ...
        if self.mode == "xy_dir":
            ...
            if dir.shape[-1] == 1:
                dir = torch.cat([dir.cos(), dir.sin()], dim=-1)
            pos_out = torch.cat([self.pe_xy(xy[..., 0]), self.pe_xy(xy[..., 1]), dir[..., [0]], dir[..., [1]]], dim=-1)
        elif self.mode == "pe_xy_dir":
            if dir.shape[-1] == 1:
                dir = torch.cat([dir.cos(), dir.sin()], dim=-1)
            pos_out = torch.cat(
