...
def repack_episode_agents_no_sim(
    ...
    episode_reduced[prefix + "agent_no_sim/z"] = np.zeros([n_step, n_agent_no_sim, 1], dtype=np.float32)
    ...
    for i, idx in enumerate(np.where(mask_no_sim)[0]):
        ...
        episode_reduced[prefix + "agent_no_sim/z"][:, i] = episode[prefix + "agent/pos"][:, idx, 2:3]
...
def repack_episode_agents(
    ...
    episode_reduced[prefix + "agent/z"] = np.zeros([n_step, n_agent, 1], dtype=np.float32)  # z
    ...
    for i, idx in enumerate(np.where(mask_sim)[0]):
        ...
        if valid.sum() > 1:
            ...
            f_z = interp1d(valid_steps, episode[prefix + "agent/pos"][valid, idx, 2:3], axis=0)
            ...
            episode_reduced[prefix + "agent/z"][step_start : step_end + 1, i] = f_z(x)
            ...
            episode_reduced[prefix + "agent/z"][valid_step, i] = episode[prefix + "agent/pos"][valid_step, idx, 2:3]
...
def filter_episode_agents(
    ...
    # if prefix == "":
    #     mask_agent_short = (agent_valid.sum(axis=0) <= 20) & ~(relevant_agent)
    #     agent_valid[:, mask_agent_short] = False
    ...
    # ! filter parking vehicles that have large distance to relevant agents and lanes, Testing only
    mask_veh_lane = (episode_reduced["map/type"][:, dim_veh_lanes].any(axis=-1, keepdims=True)) & episode_reduced[
        "map/valid"
    ]
    pos_veh_lane = episode_reduced["map/pos"][mask_veh_lane]  # [?, 2]
